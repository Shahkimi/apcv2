<x-dashboard-layout :title="$title" role="admin">
    <div class="mx-auto max-w-[1600px] space-y-6">
        <x-page-header :title="$title" :subtitle="__('System-wide operations snapshot')" />
        ...
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 space-y-4 xl:col-span-8">
                <x-chart-panel :title="__('Sales / revenue trend')" id="dash-sales-chart" />

                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <x-chart-panel :title="__('Revenue breakdown')" id="dash-category-chart" />
                    <x-recent-table
                        :title="__('Recent activity')"
                        :columns="[__('ID'), __('Event'), __('Date'), __('Type')]"
                        :rows="$recentRows"
                    />
                </div>
            </div>
            ...
            <div class="col-span-12 space-y-4 xl:col-span-4">
                ...
                <div class="rounded-xl border border-border bg-card p-4 shadow-sm">
                    <h3 class="mb-3 text-sm font-semibold text-card-foreground">{{ __('Regional mix') }}</h3>
